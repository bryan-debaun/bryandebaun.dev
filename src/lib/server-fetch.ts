...
export async function fetchWithFallback(
    ...
    try {
        ...
        if (typeof err === 'object' && err !== null) {
            ...
            if (
                ...
                if (typeof path === 'string' && path.startsWith('/api/mcp/')) {
                    const mcpBase = (process.env.MCP_BASE_URL || '').replace(
                        /\/+$/u,
                        '',
                    );
                    if (mcpBase) {
                        ...
                        if (mcpKey) {
                            upstreamInit.headers = {
                                ...(upstreamInit.headers as Record<
                                    string,
                                    unknown
                                >),
                        ...
                        try {
                            console.info(
                                'fetchWithFallback: retrying directly to MCP upstream',
                                { url, hasAuth: Boolean(mcpKey) },
                            );
                            return await makeRequest(url, upstreamInit);
                        } catch (err2) {
                            console.error(
                                'fetchWithFallback upstream retry failed',
                                { path, url, err: err2 },
                            );
                            try {
                                (err as Record<string, unknown>).retryError =
                                    err2;
                            } catch {
                                /* ignore */
                            }
                ...
                try {
                    ...
                    retryErr = err2;
                    console.error('fetchWithFallback retry failed', {
                        path,
                        origin,
                        err: err2,
                    });
                ...
                if (retryErr) {
                    try {
                        (err as Record<string, unknown>).retryError = retryErr;
                    } catch {
                        /* ignore */
                    }
                }
        ...
        if (debug) {
            ...
            const retryInfo = errRec.retryError
                ? String(errRec.retryError)
                : undefined;
            body.debug = retryInfo
                ? `${original}; retry: ${retryInfo}`
                : original;
        }
