    ...
    return (
        <div className={`overflow-x-auto ${className ?? ''}`}>
            <table className="min-w-full">
                ...
                <tbody
                    className="bg-[var(--background)] dark:bg-[var(--color-bg-dark)] divide-y"
                    style={{ borderColor: 'var(--tw-prose-td-borders)' }}
                >
                    {table.getRowModel().rows.map((row) => {
                        const rowData = row.original as T;
                        const clickable = typeof onRowClick === 'function';
                        const baseClasses =
                            'bg-[var(--color-norwegian-50)] dark:bg-[var(--background)]';
                        ...
                        return (
                            ...
                            >
                                {row.getVisibleCells().map((cell) => (
                                    <td
                                        key={cell.id}
                                        className={`px-4 py-4 whitespace-nowrap text-sm text-[var(--foreground)] ${cellClassNameFrom(cell.column.columnDef) ?? ''}`}
                                    >
                                        {flexRender(
